...
func expectedPort(t *testing.T, expected string, lsn zlgame_rpc.Acceptor) {
...
func TestGRPCZLGameRpcCommunication(t *testing.T) {
	...
	fn := func(zlgsock zlgame_rpc.ZLGameChan) {
		defer zlgsock.Close()

		for {
			var m zlgame_rpc.ZLGameMessage
	...
	var rm zlgame_rpc.ZLGameMessage
