...
func TestExtract(t *testing.T) {
	...
	Info("info proto without request ID")
	Extract(ctx).Info("info proto with request ID")
}
...
func Test(t *testing.T) {
	...
	if V(ErrorLevel, DefaultLogger) {
		Errorf("Message retry limit reached, discarding: %v %d %d", 11111, 22222, 33333)
	}
}
