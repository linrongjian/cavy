...
import (
	...
	gamemq "fastgameserver/framework/network/mq"
	gamews "fastgameserver/framework/network/websocket"
	"fastgameserver/framework/protocol/pb"
	"fmt"
	"sync"
	"sync/atomic"
	"time"
...
// NewPlayer 创建用户
func NewPlayer(playerID string, session string, conn *gamews.GWsConn) *Player {
	lock.Lock()
	defer lock.Unlock()
	...
	mqChannel, err := gamemq.NewChannel(gamemq.TopicChannelType, "")
	if err != nil {
	...
	// 注意，这里是单服通知
	subKey := fmt.Sprintf("allplayer-%v", game.Conf.ServerID)
...
// 用户接收推送函数
func (p *Player) onRecvPush(value gamemq.Delivery) {
