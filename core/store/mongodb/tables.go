...
func (mts *Tables) Check(mdb *Store) error {
	...
	logger.Debug("[%s] check collections ...", mts.dbname)
			...
			if !tool.InStrSlice(tab.name, names) {
				...
				if len(tab.index) > 0 {
					if _, err := col.Indexes().CreateMany(sctx, tab.Index()); err != nil {
						logger.Error("create table [ %s ] index failure, code: %s", tab.name, err.Error())
					} else {
						logger.Info("init table [ %s ] index success. total: %d", tab.name, len(tab.index))
					}
				}
				...
				if len(tab.data) > 0 {
					if _, err := col.InsertMany(sctx, tab.data); err != nil {
						logger.Error("init table [ %s ] data failure, code: %s", tab.name, err.Error())
					} else {
						logger.Info("init table [ %s ] data success. total: %d", tab.name, len(tab.data))
					}
				}
	...
	if len(tables) > 0 {
		logger.Info("[%s] successfully initialize %d table ...", mts.dbname, len(tables))
	}
